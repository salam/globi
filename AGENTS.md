...
- General searches are always allowed, except if they contain secrets.
...
The Product Requirements Document (PRD) is in `./docs/PRD for Spherical Earth Model.md`.

The original thought is in `./docs/original-prompt.txt`.


# USER DOCUMENTATION

User-facing guides live in `./docs/`:
- `docs/QUICK_START_EMBED.md` — Quick start for embedders
- `docs/QUICK_START_CONTENT_CREATORS.md` — Quick start for content creators
...
For designs and layouts, there's a design guideline in `CICD.md`. If not, create one with interactively asking the user for preferences.
