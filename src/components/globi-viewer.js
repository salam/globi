...
import { ContextMenu } from '../accessibility/contextMenu.js';
...
const TEMPLATE = `
  <style>
    ...
    /* Dot indicator — fades in when north pole faces the camera */
    .compass::after {
      content: '';
      position: absolute;
      top: 50%;
    ...
    .compass-arrow {
      ...
      height: 24px;
      transform: translate(-50%, -50%)
                 rotateZ(var(--compass-arrow-rotation, 0deg))
...
export class GlobiViewerElement extends HTMLElement {
  static get observedAttributes() {
    return ['language', 'planet', 'theme', 'inspect-mode', 'projection', 'loading'];
  }
  ...
  connectedCallback() {
    ...
    // Listen for toggleLegend from controller (triggered by agent API)
  ...
  attributeChangedCallback(name, _oldValue, newValue) {
    ...
    if (name === 'loading') {
      this.#controller.setLoading(newValue !== null);
    }
  }
  ...
  #applyMarkerFilter(value) {
    ...
    if (filters.length === 0 || value === 'all') {
      ...
      this.#controller.filterArcs(null);
      this.#controller.filterPaths(null);
    ...
    const matchingIds = markers
      .filter((m) => categoryMatch(m.category ?? 'default'))
      .map((m) => m.id);

    const matchingArcIds = arcs
      .filter((a) => categoryMatch(a.category ?? ''))
      .map((a) => a.id);

    const matchingPathIds = paths
      .filter((p) => categoryMatch(p.category ?? ''))
      .map((p) => p.id);

    this.#controller.filterMarkers(matchingIds);
    this.#controller.filterCallouts(matchingIds);
    this.#controller.filterArcs(matchingArcIds);
    this.#controller.filterPaths(matchingPathIds);
  ...
  #updateNavigationHud() {
    ...
    if (this.#compassArrow) {
      ...
      this.#compassArrow.style.setProperty('--compass-arrow-rotation', `${rotation.toFixed(1)}deg`);
      this.#compassArrow.style.setProperty('--compass-arrow-tilt', `${(-tilt).toFixed(1)}deg`);
      const compassEl = this.#compassArrow.parentElement;
      if (compassEl) {
        compassEl.style.setProperty('--compass-dot-opacity', dotOpacity.toFixed(2));
      }
    }
...
export function registerGlobiViewer() {
  if (!customElements.get('globi-viewer')) {
    customElements.define('globi-viewer', GlobiViewerElement);
  }
}
